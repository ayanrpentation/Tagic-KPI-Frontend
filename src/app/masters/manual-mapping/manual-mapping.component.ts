...
export class ManualMappingComponent implements OnInit {
  ...
  selectedFileTypeList = [] as any;
  dropdownSettingsFileType:IDropdownSettings = {};
  mappingFileList = [{"status":1},{"status":2},{"status":3}] as any;
  // January, February, March, April, May, June, July, August, September, October, November, December
  monthData = {1:'January', 2:'February',3:'March', 4:'April', 5:'May', 6:'June',7:'July', 8:'August', 9:'September', 10:'October', 11:'November',12:'December'} as any;
  intervalId:any;
  ...
  ngOnInit(): void {
    ...
    this.dropdownSettingsFileType = { idField: 'id',textField: 'details_description',allowSearchFilter: true,enableCheckAll:true, itemsShowLimit: 1 };
  ...
  rerunFile(rerunData:any){
    console.log(rerunData);
    const data = {
      'userId': this.userId,
      'year':rerunData.year,
      'month':rerunData.month,
      'channel':rerunData.channel_new_name,
      'channelId':rerunData.channel_new_id,
      'vertical':rerunData.vertical,
      // 'fileType':this.uploadFileType,
      // 'fileTypeId': this.mappingFileTypeId,
      'fileTypeId' : [{'id':rerunData.file_type_id,'details_description':rerunData.file_type}],
      'rowId':rerunData.id,
      'bathcId':sessionStorage.getItem('batchId')
      }
    ...
    this.rest.mappingCall(data).subscribe((res: any) => {
      ...
      if (res.success) {
        ...
        // this.mapping = false;
        // window.alert(res.message)
        // sessionStorage.setItem("batchId", res.batch_id);
  ...
  mappingCall(){
      ...
      else if(this.selectedFileTypeList.length == 0){
        window.alert("File Type is not selected")
        return
      }
      ...
      const data = {
        ...
        // 'fileType':this.uploadFileType,
        // 'fileTypeId': this.mappingFileTypeId,
        'fileTypeId' : this.selectedFileTypeList}
      
  
      // this.mapping = true;
      
      this.rest.mappingCallAll(data).subscribe((res: any) => {
        // this.ngxService.start();
        if (res.success) {
          this.ngxService.stop();
          this.notifier.notify('success', res.message);
  ...
  openModal(logModal: any) {
    // this.userName = '';
    // this.existReport = '';
    this.intervalId = setInterval(() => {
      this.getLog();
    }, 5000);
    this.modalService.open(logModal, {centered: true, size: 'xl', scrollable: true, backdrop:false});
  }
  ...
  getLog(){
    ...
    this.rest.getLog(data).subscribe((res: any) => {
      // this.ngxService.start();
      if (res.success) {
        // this.ngxService.stop();
        this.mappingFileList = res.result;
        // this.mapping = false;
        // window.alert(res.message)
