...
export class UploadAllFileComponent implements OnInit {
  ...
  @ViewChild('fileUp') fileInput!: ElementRef;

  fileId = 0 as any;
  filetypeId = 0 as any;

  eventFile_upload : any;
  error = '' as any;
  ...
  uploadYear = this.ActivatedRoute.snapshot.paramMap.get('year') as any;
  uploadMonth = this.ActivatedRoute.snapshot.paramMap.get('month') as any;
  uploadChannel = this.ActivatedRoute.snapshot.paramMap.get('channelId') as any;
  ...
  showFileExist_popup = false as any
  ...
  uploadFileAddToExisting(event: any): any {
    ...
    // this.eventFile_upload = event.target.files;
    this.eventFile_upload = event
  ...
  checkFileExist(){
      ...
      const data = {
        ...
        uploadMonth: this.uploadMonth,
        channelId: this.uploadChannel,
        purpose: 'to check if any data exists for this module for the same month-year'
      }


      this.rest.checkFileExist_status(data).subscribe((res: any) => {
        if(res.success){

          this.fileId = res.fileId
          this.filetypeId = res.fileTypeId

          if(res.fileExist == false){

          }

          else if(res.fileExist == true){
            this.showFileExist_popup = true
          }
  ...
  closeFileExist_popup(){
    this.showFileExist_popup = false
    
  }

  addMore(){

    this.uploadFileAddToExisting(this.eventFile_upload)
  ...
  replaceFile(){
    ...
    const data = {
      ...
      channelNewId: this.uploadChannel,
      userId: this.userId,
    }
