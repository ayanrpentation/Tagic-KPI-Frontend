...
export class ModulecreationComponent implements OnInit {
  ...
  // fileType creation section
  selectedExactFiletype = '' as any
  fileTypeName = "" as any;
  fileTypeName_short = "" as any;
  channelNewList = [] as any;
  selectedChannel = sessionStorage.getItem("defaultChannelForAll") as any;
  createdFileTypeId = "" as any;
  ...
  uploadFile(event: any): any {
    ...
    fd.append('channelId', this.selectedChannel);
    fd.append('fileTypeId', this.createdFileTypeId);

    fd.append('uploadMonth', this.uploadMonth);
    fd.append('uploadYear', this.uploadYear);

    // file_type: this.selectedExactFiletype,
    fd.append('exact_file_type', this.selectedExactFiletype)
...
createFileType(){
  if(this.selectedChannel == ''){
    this.notifier.notify('error', 'Select Channel Name');
    return;
  }
  if(this.fileTypeName == ''){
    this.notifier.notify('error', 'Enter File Name');
    return;
  }
  if(this.selectedExactFiletype == ''){
    this.notifier.notify('error', 'Select File Type');
    return;
  }
  ...
  const data = {
    ...
    details_description: this.fileTypeName,
    exact_file_type: this.selectedExactFiletype,
    details_code: this.fileTypeName_short,
    details_active: 1
  }
this.rest.insertdetails(data).subscribe((res: any) => {
  if (res.success) {
     this.createdFileTypeId = res.fileTypeId;
     this.notifier.notify('success', "File Type Created Successfully");
  }
  else{
