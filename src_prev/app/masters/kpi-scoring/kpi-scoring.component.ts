...
export class KpiScoringComponent implements OnInit {
  ...
  constructor(private rest: RestApiService, private modalService: NgbModal, private notifier: NotifierService, private ngxService: NgxUiLoaderService,) { }
  ...
  ngOnInit(): void {
    ...
    this.channelNew = sessionStorage.getItem("defaultChannelForAll") as any;
  ...
  downloadScore(flag: any){
    ...
    if(this.periodType == 'month' || this.periodType == 'ytd'){
      if(this.selectedMonthList.length == 0 ){
        ...
        this.errormsgstatus = true
        this.errormsg = 'Please Select Channel Name.'
