...
@Component({
  selector: 'app-maper-master',
  templateUrl: './maper-master.component.html',
  styleUrls: ['./maper-master.component.css']
})
export class MaperMasterComponent implements OnInit {
  dataList: any = [];
  showAction = -1;
  columnList: any = [];
  columnName = 'all';
  ...
  ngOnInit(): void {
    this.moduleId = this.common.getMapperMasterId();
    this.getColumnName();
    this.genericFilter();
  }
  getColumnName() {
  ...
  uploadFile(event: any): any {
    ...
    this.rest.uploadMapperMaster(fd).subscribe((res: any) => {
      if (res.success) {
  ...
  addMapper(){
    this.router.navigate(['dashboard/addmappermst']);
  }
  editMapper(itemId: any, levelEmployee: any,designationId:any,empCode:any){
    if (this.isedit == false) {
      this.isedit = true;
      this.editId = itemId;
      this.editlevelEmployee = levelEmployee;
      this.editdesignation_type = designationId;
  ...
  changeValue(id: any) {
    if (this.editId == id) {
      const data = {
        'id': id,
        'levelEmployee': this.editlevelEmployee,
        'designationId': this.editdesignation_type,
        'empCode': this.editempCode,
      }
      this.rest.editMapperMaster(data).subscribe((res: any) => {
        if (res.success) {
          this.notifier.notify('success', res.message);
          this.isedit = false;
          this.genericFilter();
