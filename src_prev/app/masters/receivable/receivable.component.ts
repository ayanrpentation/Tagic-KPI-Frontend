...
export class ReceivableComponent implements OnInit {
  ...
  downloadTemplate(){
    window.open(this.rest.file_path + '/template/Receivable_Pending_template.xlsx');
  }
  ...
  getRecv(flag: number = 0): any {
    ...
    this.rest.resvmst_fetch(data).subscribe((res: any) => {
      if (res.success) {
  ...
  uploadFile(event: any): any {
    ...
    fd.append('fileType', 'RP');
    fd.append('branchId', this.branchId);
    fd.append('userId', this.userId);
    // fd.append('companyId', this.common.getCompanyId());
    this.ngxService.start();
    this.fileUploadSub = this.rest.receivable_pending(fd).subscribe((res: any) => {
